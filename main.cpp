/*
 * ring_allreduce_advanced.c
 *
 * Ring Reduce-Scatter, All-Gather, and All-Reduce using Linux RDMA Verbs API.
 * Features:
 *  - Full Eager vs. Rendezvous protocol comparison path
 ...
 *
 * Example usage:
 *    ./ring_allreduce -myindex 01 -list host1 host2 -protocol rendezvous -count 65536
 */
...
#define PG_PIPELINE_STAGES    2     /* Double-buffering for pipeline overlap */
#define PG_MICRO_CHUNK_SIZE   4096  /* Micro-chunk size in elements for pipelining */
...
#define WRID_EAGER_RECV       0x1002ULL
#define WRID_WRITE_SEND       0x1003ULL
#define WRID_WRITE_RECV       0x1004ULL
...
typedef struct {
    ...
    int ib_port; //physical RDMA port, usually 1
    int gid_index; //GID table entry
    PROTOCOL_MODE protocol; //eager or rendezvous
    char hosts[PG_MAX_HOSTS][PG_MAX_HOST_LEN]; //list of all participating machines
} pg_config_t;
...
typedef struct {
    ...
    struct ibv_context *context; //opened RDMA device
    ...
    /* Pre-allocated double-buffer for pipelined eager transfers */
    void *pipe_buffers[PG_PIPELINE_STAGES];
    struct ibv_mr *pipe_mrs[PG_PIPELINE_STAGES];
    ...
    size_t bytes)
{
  ...
  uint8_t *slot_buffer =
      ...
      (size_t)slot_index * pg->pipe_buffer_size;
    ...
    int *owned_chunk)
{
  ...
  for (int step = 0; step < P - 1; ++step) {
    ...
    while (offset < chunk_bytes) {
      size_t remaining = chunk_bytes - offset;
      size_t current_bytes =
          remaining > micro_chunk_bytes
          ? micro_chunk_bytes
          : remaining;

      size_t current_elems = current_bytes / elem_size;
      ...
      uint8_t *received_data =
          (uint8_t *)pg->eager_recv_pool +
          (size_t)completed_slot * pg->pipe_buffer_size;
      ...
      /* Keep the receive queue permanently populated. */
      if (post_eager_receive_slot(
          pg,
          completed_slot,
          pg->pipe_buffer_size) != 0) {
        return -1;
      }
                  ...
                  DATATYPE datatype, OPERATION operation, void *pg_handle)
{
  ...
  if (pg->cfg.protocol == PROTOCOL_EAGER &&
      chunk_bytes > pg->pipe_buffer_size) {
    fprintf(stderr,
            "Rank %d: eager chunk size %zu exceeds "
            "pipeline buffer size %zu\n",
            pg->cfg.rank,
            chunk_bytes,
            pg->pipe_buffer_size);
    return -1;
                            ...
                            const qp_info_t *remote)
{
  ...
  /*
   * Use a conservative MTU while debugging.
   */
                          ...
                          void **pg_handle)
{
  ...
  *pg_handle = NULL;
  ...
  /* -------------------------------------------------------------- */
  /* Allocate and register pipeline buffers                         */
  /* -------------------------------------------------------------- */
  ...
  for (int i = 0;
    ...
    if (rc != 0) {
      fprintf(stderr,
              "Rank %d: posix_memalign buffer %d "
              "failed: %s\n",
              pg->cfg.rank,
              i,
              strerror(rc));
    ...
    memset(pg->pipe_buffers[i],
           0,
           pg->pipe_buffer_size);

    pg->pipe_mrs[i] = ibv_reg_mr(
        pg->pd,
        pg->pipe_buffers[i],
        pg->pipe_buffer_size,
        IBV_ACCESS_LOCAL_WRITE |
  ...
  *pg_handle = pg;
  ...
  if (pg) {
    ...
    for (int i = 0;
      ...
      if (pg->pipe_mrs[i]) {
        ibv_dereg_mr(pg->pipe_mrs[i]);
      }

      free(pg->pipe_buffers[i]);
    }
                                ...
                                size_t output_size)
{
  if (bytes >= 1024U * 1024U) {
    snprintf(output, output_size, "%.2f MiB",
             (double)bytes / (1024.0 * 1024.0));
  }
  else if (bytes >= 1024U) {
    snprintf(output, output_size, "%.2f KiB",
             (double)bytes / 1024.0);
  }
  else {
...
int main(int argc, char **argv)
{
   ...
   *
   *   chunk_bytes <= pipe_buffer_size
   *
   * With two processes and a 131072-byte pipeline buffer,
  ...
  for (size_t size_index = 0;
    ...
    /*
     * The current eager All-Gather receives an entire chunk into one
     * pipeline buffer.
     */
    if (chunk_bytes > pg->pipe_buffer_size) {
      if (g_config.rank == 0) {
        fprintf(stderr,
                "Skipping %zu bytes: chunk size %zu "
                "exceeds eager buffer size %zu\n",
                message_bytes,
                chunk_bytes,
                pg->pipe_buffer_size);
      }
 ...
 */
  ...
  if (g_config.rank == 0) {
    ...
    for (size_t size_index = 0;
      ...
      printf(
          "%12zu | %12.2f | %12.2f | %12.6f | %12.6f | %12s\n",
          message_sizes_bytes[size_index],
          eager_times_us[size_index],
          rendezvous_times_us[size_index],
          eager_bandwidth_gbps[size_index],
    ...
    if (stable_threshold != 0) {
      ...
      printf(
          "\nPossible stable protocol threshold: %s (%zu bytes)\n"
